...
export class TurnosService {
  ...
  async create(dto: CreateTurnoDto): Promise<Turno> {
    ...
    // Validación optimizada de horarios
    const horarioValido = await this.claseRepository
      .createQueryBuilder('clase')
      .leftJoin('clase.horarios', 'horario')
    ...
    const turno = this.turnoRepository.create({
      ...
      horaInicio: dto.horaInicio,
      horaFin: dto.horaFin,
      estado: EstadoTurno.CONFIRMADO, // 👈 Confirmar automáticamente
    });

    const turnoGuardado = await this.turnoRepository.save(turno);

    // ✅ Enviar email de prueba de forma ASÍNCRONA (no bloquear la respuesta)
    this.sendTestEmailOnBooking(usuario.email, usuario.nombre, clase.nombre)
      .catch(error => {
  ...
  private async sendTestEmailOnBooking(userEmail: string, userName: string, claseNombre: string) {
