...
export class ScheduleService {
  ...
  async handleTurnosReminder() {
    ...
    try {
      ...
      console.log(`📋 Encontrados ${turnosActivos.length} turnos activos para recordar`);
      ...
      if (turnosActivos.length > 0) {
        console.log(`✅ Enviados ${turnosActivos.length} recordatorios de turnos`);
      }
    } catch (error) {
  ...
  async handleFiveMinuteWarning() {
    ...
    try {
      ...
      if (turnosProximos.length > 0) {
        console.log(`✅ Enviadas ${turnosProximos.length} notificaciones de 5 minutos`);
      }
    } catch (error) {
  ...
  async sendTestEmailOnBooking(userEmail: string, userName: string, claseNombre: string) {
  ...
  private async sendTurnoReminderEmail(turno: Turno) {
    ...
    try {
      ...
      } else if (diferenciaTiempo < 60 * 60 * 1000) { // Menos de 1 hora
        const minutosRestantes = Math.ceil(diferenciaTiempo / (1000 * 60));
        tiempoRestante = `⏰ Faltan ${minutosRestantes} minutos`;
      } else if (diferenciaTiempo < 24 * 60 * 60 * 1000) { // Menos de 1 día
      ...
      console.log('✅ Recordatorio enviado a:', turno.user.email, '- Clase:', turno.clase.nombre);
    } catch (error) {
